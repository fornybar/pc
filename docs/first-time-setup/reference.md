...
- **Partitions:** Erase disk. We recommend swap with no hibernate. No swap avoids creating swap partition, but pc will crash when reaching memory limit since there is no swap to use for offloading. For swap with hibernate you must disable random encryption.
- Swap partitions with encryptions must either be in encryption config (see step 3.2) or have `randomEncryption.enable = true` in `swapDevices` entry.
...
After creating new keys with `sbctl create-keys` they must be enrolled in order to use for secure boot.
...
`Vendor Keys` should say `microsoft builtin-db builtin-db builtin-KEK`. If not you need to [enter Setup Mode](#entering-setup-mode) and enroll keys.
