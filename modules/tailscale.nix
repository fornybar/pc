    ...
    text = ''
      ...
      update_ssh_config() {
        ...
        peers=$(printf '%s' "$status" \
          | jq -r '.Peer[] | select(.Online) | [.HostName, ([.TailscaleIPs[] | select(contains(":") | not)][0] // .TailscaleIPs[0])] | @tsv' 2>/dev/null) || true
...

        {
          ...
          echo ""
          if [ -n "$load_key_cmd" ]; then
            echo "Match host $host_list exec \"$load_key_cmd\""
            echo ""
          fi
        ...
        echo "Updated $config_file ($ssh_user@{$host_list})"
